...
in
{
  ...
  config =
    let
      mkSecretName = name: if cfg.mapping.${name} then name else cfg.mapping.${name};
    in
    {
      home-manager.users = mapAttrs (user: _: {
        modulo.secrets.values = genAttrs config.home-manager.users.${user}.modulo.secrets.applications (
      ...
      sops = {
        ...
        secrets = mapAttrs' (name: nameValuePair (mkSecretName name)) cfg.applications;
      };
      ...
      modulo.secrets = {
        ...
        values = mapAttrs (name: _: config.sops.secrets.${mkSecretName name}.path) cfg.applications;
      };
    };
}
