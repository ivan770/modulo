{
  ...
  colors = ["background" "foreground" "highlight"];
...
in {
  options.modulo.desktop.dunst = {
        ...
        (genAttrs colors (color:
          mkOption {
            ...
            default =
              ...
              then null
              else cfg.settings.${defaultLevel}.${color};
            defaultText =
              if level == defaultLevel
              then "null"
              else "config.modulo.desktop.dunst.${defaultLevel}.${color}";
            description = ''
    ...
    extraConfig = mkOption {
      type = with types;
        attrsOf (
          attrsOf (either str (either bool (either int (listOf str))))
        );
      default = {};
      description = ''
        Extra configuration.
  ...
  config.services.dunst = mkIf cfg.enable {
    ...
    settings = let
      mkConfig = prefix: val: listToAttrs (map (level: nameValuePair "${prefix}_${level}" (val level)) urgencyLevels);

      colorConfig = mkConfig "urgency" (
        level:
          listToAttrs (
            map
            (color: nameValuePair color (mkIf (cfg.settings.${level}.${color} != null) cfg.settings.${level}.${color}))
            colors
          )
      );
      ...
      soundConfig = mkConfig "sound" (
        level:
          mkIf (cfg.settings.${level}.sound != null) {
            msg_urgency = level;
            script = toString (pkgs.writeShellScript "${level}-notification" ''
    ...
    in
      recursiveUpdate ({
          global = {
            ...
            offset = concatStringsSep "x" (map toString [
